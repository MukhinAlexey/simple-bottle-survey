...
def login():
    if not admin_flag:
        login_template = template('templates/login.html')
...
@bottle.route('/survey')
def survey():
    survey_template = template('templates/index.html')
...
def dashboard():
    if admin_flag is True:
        ...
        dashboard_template = template('templates/dashboard.html', scores=scores)
